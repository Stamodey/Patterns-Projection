...
puts "Есть ли контакт: #{student1_short.contact_available? ? 'Да' : 'Нет'}"
puts

puts "--- Студент2Short ---"
puts student2_short.to_s
puts "Есть ли контакт: #{student2_short.contact_available? ? 'Да' : 'Нет'}"
puts

# Создаем экземпляр Data_list_student_short
data_list = Data_list_student_short.new([student1_short, student2_short])
...
puts "--- Таблица данных ---"
puts "Количество строк в таблице: #{table.rows_count}"
puts "Количество столбцов в таблице: #{table.columns_count}"
puts "Первый элемент таблицы: #{table.get_element(0, 1)}"
puts
